...
export class PhoneVerificationPage implements OnInit, AfterViewInit {
  ...
  async ngAfterViewInit() {
    this.loading = true;
    this.clientObj = await this.authService.getFieldDataFromStorage('driver-auth');
  ...
  getVerificationCode(sendStatus?: string) {
    ...
    this.driverAuthService.forgotPassword(this.clientObj)
      .subscribe(
        response => {
          this.loading = false;
  ...
  verifyCode() {
    ...
    this.driverAuthService.verifyForgotPasswordCode(this.clientObj)
      .subscribe(
        response => {
          this.loading = false;
